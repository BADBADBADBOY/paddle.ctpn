    ...
    def detect(self, img_file,model=None):
        ...
        with paddle.no_grad():
            if model is None:
                ...
                pre_score, pre_reg,= model(img)
        score = pre_score.reshape((pre_score.shape[0], 10, 2, pre_score.shape[2], pre_score.shape[3])).squeeze(0).transpose((0, 2, 3, 1)).reshape((-1, 2))
        score = F.softmax(score, 1)
        
        score = score.reshape((10, pre_reg.shape[2], -1, 2))
        pre_score = score.transpose((1, 2, 0, 3)).reshape([pre_reg.shape[2], pre_reg.shape[3], -1]).unsqueeze(0).cpu().numpy()
...
,gt_path='/home/aistudio/work/icdar/aistudio/work/data/icdar/test_gt'):
    ...
    for file in files:
        ...
        boxes, text_proposals, rh, rw = valdetect(im_file,model)

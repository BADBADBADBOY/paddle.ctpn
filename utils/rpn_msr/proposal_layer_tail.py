...
def proposal_layer(rpn_cls_prob_reshape,tail_score,rpn_bbox_pred,im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    """
    Parameters
    ----------
    ...
    tail_score = np.reshape(np.reshape(tail_score, [1, height, width, _num_anchors, 2])[:, :, :, :, :],[1, height, width, 2*_num_anchors])
    ...
    tail_score = tail_score.reshape((-1,2))
    label_anchors = np.argmax(tail_score,1)
    tail_score = tail_score[range(label_anchors.shape[0]),label_anchors]
    label_anchors = label_anchors.reshape((-1, 1))
    ...
    scores = scores[keep]
    label_anchors = label_anchors[keep]
    ...
    scores = scores[order]
    label_anchors = label_anchors[order]
    ...
    # batch inds are 0
    blob = np.hstack((label_anchors.astype(np.float32, copy=False),scores.astype(np.float32, copy=False), proposals.astype(np.float32, copy=False)))

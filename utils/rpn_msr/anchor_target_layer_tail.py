...
from utils.rpn_msr.draw import draw_fg,draw_class,draw_bg
import pdb
...
def anchor_target_layer(image,rpn_cls_score, gt_boxes, im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    """
    try:
        img_auchor = image.copy()
        for item in gt_boxes:
            color = (0,0,255)
            img_auchor = cv2.rectangle(img_auchor,(int(item[0]),int(item[1])),(int(item[2]),int(item[3])),color)
        cv2.imwrite('result_auchor.jpg',img_auchor)
    except:
        print('warning!!!!!')
    ...
    _typesAll =  np.tile(_types,(anchors.shape[0],1))
    ...
    max_overlaps_types = _typesAll[np.arange(len(inds_inside)), argmax_overlaps]
    ...
    gt_overlaps_types = _typesAll[gt_argmax_overlaps,np.arange(overlaps.shape[1])]
    
 
    gt_argmax_overlaps = np.where(overlaps == gt_max_overlaps)[0]
    gt_argmax_overlaps_dontcare = gt_argmax_overlaps_dontcare[ np.where(overlaps == gt_max_overlaps)[1]]
    gt_overlaps_types = gt_overlaps_types[ np.where(overlaps == gt_max_overlaps)[1]]
    ...
    labels[gt_argmax_overlaps[gt_argmax_overlaps_dontcare==1]] = gt_overlaps_types[gt_argmax_overlaps_dontcare==1] # 每个位置上的9个anchor中overlap最大的认为是前景
    labels[(max_overlaps >= cfg.RPN_POSITIVE_OVERLAP) & (max_overlaps_dontcare==1)] = max_overlaps_types[(max_overlaps >= cfg.RPN_POSITIVE_OVERLAP) & (max_overlaps_dontcare==1)] # overlap大于0.7的认为是前景
    ...
    try:
        ...
        print('warning!!!')
    #####################################################  
    # draw_fg(image, labels, anchors)
    # draw_bg(image, labels, anchors)
    # draw_class(image, class_labels, anchors)

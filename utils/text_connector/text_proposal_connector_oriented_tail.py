...
class TextProposalConnector:
    ...
    def get_text_lines(self, text_proposals, scores, label_anchors,im_size):
        """
        ...
        tp_groups = split_tp_groups_by_anchor_labels_new(tp_groups,label_anchors)
...
def split_tp_groups_by_anchor_labels_new(tp_groups,label_anchors):
    new_group = []
    # pdb.set_trace()
    for item in tp_groups:
        label = label_anchors[np.array(item)]
        label = label.reshape(-1).tolist()
        all_item = get_tp_group_new(label,item)
        new_all_item = []
        for value in all_item:
            if(len(value)>1):
                new_all_item.append(value)
        new_group.extend(new_all_item)
    return new_group
